#!/bin/bash
# ================================================
# Ubuntu mihomo + WebUI 最终版 2.0（Tommy 专属）
# 功能：订阅自动最优节点（可选） + mc 面板重新选最优 + 关闭全局代理
# ================================================

set -e

echo "🚀 开始安装 mihomo + WebUI 最终版 2.0 ..."
...
while true; do
    ...
    case $choice in
        ...
        11)
            echo "🔄 重新测试最优节点..."
            if [ -n "$SECRET" ]; then
                curl -X DELETE "http://127.0.0.1:$CTRL_PORT/group/🚀 自动最优" -H "Authorization: Bearer $SECRET" -s
            else
                curl -X DELETE "http://127.0.0.1:$CTRL_PORT/group/🚀 自动最优" -s
            fi
            echo "✅ 已触发重新选择最优节点（url-test 会自动选最快）"
            ;;
        12)
...
echo ""
echo "🎉 mihomo 2.0 最终版安装完成！"
echo "========================================"
echo "WebUI: http://127.0.0.1:$CTRL_PORT/ui"
echo "混合代理: 127.0.0.1:$MIXED_PORT"
if [ "$AUTO_BEST" = "yes" ]; then
    echo "✅ 自动最优节点已启用（订阅加载后会自动选最快节点）"
fi
echo ""
echo "🔥 现在输入：mc   ← 使用控制面板（含重新选最优、关闭全局代理、卸载等）"
echo "========================================"

if systemctl is-active --quiet mihomo; then
    echo "✅ 服务运行正常！"
    echo "建议：输入 mc → 选项 5 先更新一次订阅"
else
